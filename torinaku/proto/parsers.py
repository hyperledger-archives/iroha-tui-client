...
class ProtoMessageExplorer:
    ...
    def _parse_message_descriptor(
        self, proto_message_descriptor, message_path=None, field_path=None
    ):
        result = []
        if not message_path:
        ...
        for field in proto_message_descriptor.fields:
            ...
            if field.message_type:
                result.extend(
                    self._parse_message_descriptor(
                        field.message_type, message_path, next_field_path
                    )
                )
            else:
                result.append(
                    self._parse_field_descriptor(field, message_path, next_field_path)
                )
        return result
