...
class CheckListBox(_BaseListBox):
  ...
  A MultiListBox is a widget that displays a list from which the user can select multiple options.
  """

  def __init__(self, height, options, centre=False, label=None, name=None,
               add_scroll_bar=False, on_change=None, on_select=None, validator=None, type='check'):
  ...
  def update(self, frame_no):
    ...
    for i, (text, value) in enumerate(self._options):
      if start_line <= i < start_line + height - y_offset:
        ...
        self._frame.canvas.print_at(
            '{}{}{} '.format(borders[0], check_char if value in self._selected else ' ', borders[1]),
            self._x + self._offset,
        ...
        self._frame.canvas.print_at(
            '{:{}}'.format(
                _enforce_width(text, width - 4 - self._offset, self._frame.canvas.unicode_aware),
