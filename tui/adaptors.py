...
from asciimatics.widgets import Frame, ListBox, Layout, Divider, Text, \
    Button, TextBox, Widget, MultiColumnListBox, Label, VerticalDivider, RadioButtons, CheckBox

from proto.message import MessageModel
from tui.checklistbox import CheckListBox, RadioListBox
import re
...
class CommandTuiAdaptor:
  ...
  def __init__(self, message_model: MessageModel, frame: Frame):
    self._message_model = message_model
    self._frame = frame
    self._lay = None
    self._lay_column = None
  ...
  def draw_ui(self):
    ...
    if len(descr):
      command_name = re.sub(r'([A-Z])', r' \1', self._message_model.descriptor[0]['message_path']).strip()
    ...
    if has_optional_primitive:
      ...
      self._lay.add_widget(
          Label('* Fields, which names are enclosed into "[" and "]" are optional'),
          self._lay_column
      )
  ...
  def _draw_unknown_field(self, field_descriptor):
    self._lay.add_widget(
        Label('Unable to display field: {}{} (unsupported type)'.
              format(
