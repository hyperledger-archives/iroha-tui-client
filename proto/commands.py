...
from .helpers import shorten_command_name
...
class CommandWrapper:
  ...
  def __init__(self, unwrapped_command=None, wrapped_command=None):
    if not (bool(unwrapped_command) != bool(wrapped_command)):
      raise ValueError('Either raw command or wrapped has to be passed to CommandWrapper')
    self._wrapped = None
...
class ProtoCommandLoader:
  ...
  def commands(self):
    """
    List of tuples of pretty message name, message name and field name within Command message.
...
class CommandPreview:
  ...
  def __init__(self, unwrapped_command=None, wrapped_command=None):
    if not (bool(unwrapped_command) != bool(wrapped_command)):
      raise ValueError('Either raw command or wrapped has to be passed to CommandWrapper')
    unwrapped = unwrapped_command if unwrapped_command else wrapped_command.unwrapped
    self._message_model = MessageModel(unwrapped)
